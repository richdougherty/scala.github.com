...
layout: sip
disqus: true
title: SIP-XX - ExecutionContext Preparation
---

**By: Rich Dougherty**

<span class="label success">Updated September 16th, 2013</span>

&nbsp;

Scala 2.10.0 and 2.9.3 introduced the ExecutionContext class. An ExecutionContext
is an object that executes Runnable tasks. An ExecutionContext manages the details
of how tasks are executed, in a thread pool, use a fork-join pool, etc.
Code which executes Runnable tasks can accept an ExecutionContext and
thereby abstract away the details of how those tasks are executed.

The scala.concurrent library makes extensive use of ExecutionContexts.
Many parts of the library accept ExecutionContexts and
several ExecutionContext implementations are provided for use. All methods
that execute user code accept an ExecutionContext as an implicit argument.
Users who call this method can indicate their preferred execution strategy
by providing an implicit ExecutionContext in scope when calling these methods.

Scala's ExecutionContext is very similar to Java's Executor, but with a couple
of extra methods. This SIP concerns itself with the *prepare* method. The prepare
method  tells an ExecutionContext to capture any important local state and return
another ExecutionContext that is now *prepared* and ready for use.

An example where this method is useful is the case of an ExecutionContext that runs user
code in a thread pool and needs to make sure that the user code runs with the appropriate
context ClassLoader installed. One way of fulfilling this requirement is to create an
ExecutionContext with an appropriate ClassLoader installed within it.

class FixedClassLoaderExecutionContext(fixed: ClassLoader, delegate: ExecutionContext) extends ExecutionContext {
  def execute(r: Runnable) = {
    delegate.run(new Runnable {
      val prev = Thread.currentThread.getContextClassLoader
      Thread.currentThread.setContextClassLoader(fixed)
      try r.run() finally Thread.currentThread.setContextClassLoader(prev)   
    })
  }
}

Another option is
to create an ExecutionContext that captures the current local context ClassLoader when
its prepare method is called. The prepare method would return a second ExecutionContext with the
ClassLoader set inside it. It would use this ClassLoader when executing user code.

class UnpreparedClassLoaderExecutionContext(delegate: ExecutionContext) extends ExecutionContext {
  def prepare(): ExecutionContext = FixedClassLoaderExecutionContext(
    Thread.currentThread.getContextClassLoader,
    delegate
  )
  def execute(r: Runnable) = new IllegalStateException("Must prepare before use")
}
...
For the second option to work, it is necessary that the prepare method is called
appropriately. At the moment 

 This ExecutionContext could have the ClassLoader stored
within it when it is created. A



 but needs to capture the local context ClassLoader and install
it when running user code.

 captures
the context ClassLoader and r

that includes this state.

https://issues.scala-lang.org/browse/SI-6185


One significant difference between Scala's ExecutionContext and Java's
Executor is the additional requirement that an ExecutionContext must
be *prepared* before use. 

This 
The motivating example is this:
* ExecutionContexts are 

wishes to abstract away from such details can accept an
ExecutionContext.



ExecutionContexts abs
